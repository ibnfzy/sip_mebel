              ...
              <tbody>
                ...
                <?php foreach ($keranjang as $item): ?>
                  ...
                  <tr>
                    ...
                    <td>
                      <?= ($item['type_reward'] == 'free') ? 'Free 1 Meja' : 'Diskon' ?>
                    </td>
                    <td>Rp.
                    ...
                    <td>
                      ...
                      <div class="btn-group-vertical btn-group-sm" role="group">
                        <?php if ($item['bukti_bayar'] != null): ?>
                          <a href="#myModal<?= $i ?>" data-toggle="modal" class="btn btn-warning"><i
                              class="fa-solid fa-eye"></i>
                            Lihat Bukti Bayar</a>
                        <?php endif ?>
                        <a onclick="modalShow('<?= $item['tgl_checkout'] ?>', '<?= ($item['tanggal_upload_bayar'] == null or $item['status_bayar'] == 'Menunggu Bukti Bayar') ? 'Belum Bayar' : $item['tanggal_upload_bayar']; ?>', '<?= $get['fullname']; ?>', '<?= $q['kota']; ?>, <?= $q['kec_desa']; ?> <?= $q['alamat']; ?>', '<?= $item['rowid']; ?>', '<?= str_replace('08', '628', $q['nomor_hp']); ?>', '<?= $item['type_reward'] ?>', '<?= $item['potongan']; ?>')"
                          class="btn btn-info">
  ...
  const modalShow = (
    ...
    xmlhttp.onload = function () {
      ...
      for (let x in myObj) {
        ...
        no = 1;
        isDiskon = myObj[x].potongan;
        dis = (isDiskon === '1') ? diskon : 0;
